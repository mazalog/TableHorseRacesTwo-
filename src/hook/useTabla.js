...
export default function useTabla(nTabla) {
    ...
    const handleInputChangeJugada = (e) => {
        ...
        setTabla(tabla.map(doc => doc.id === parseFloat(e.target.dataset.id) ? { ...doc, total: parseFloat(e.target.value), [e.target.name]: parseFloat(e.target.value) } : doc))
    ...
    const save = (id) => {
        ...
        const salvar = {
            ...
            premio: parseFloat(format(totalTabla1)),
            ...
            utilidad: parseFloat(jugada.jugador === "C" ? format(totalCaballosComprados - (totalTabla1) + (totalTabla1) - totalAcumulado())
                :
                format(totalCaballosComprados - (totalTabla1) - totalAcumulado()))
        }
